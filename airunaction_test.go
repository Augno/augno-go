...
func TestAIRunActionContinueWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.AI.Runs.Actions.Continue(
		...
		openmrp.AIRunActionContinueParams{
			ContinueRunRequest: openmrp.ContinueRunRequestParam{
				...
				RejectedToolCallIDs: []string{"string"},
				RejectedToolSlugs:   []string{"string"},
			},
			Include: []string{"actions"},
		},
	)
...
func TestAIRunActionRetryWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
